...
TMP_DIR="$(mktemp -d /private/tmp/mac-editor-swift-strict.XXXXXX)"
BIN_DIR="$TMP_DIR/bin"
...
printf '%s\n' "$HOME" "$XDG_CACHE_HOME" "$CLANG_MODULE_CACHE_PATH" "$SWIFT_MODULE_CACHE_PATH" >"$STRICT_BUILD_ENV_FILE"
...
echo "swift strict build gate test: passed"
