...
usage() {
  cat <<'USAGE'
Usage: script/record_clean_runtime_evidence.sh --item <id> --summary <text> --execute
       script/record_clean_runtime_evidence.sh --dry-run

Records redacted clean-user runtime evidence. Use only after the app has
actually been launched and smoke checked from a clean macOS account or
equivalent test user.
...
fail() {
  echo "clean runtime evidence recorder: $*" >&2
  exit 1
}
...
reject_private_content() {
  ...
  if printf "%s" "$value" | grep -Eq '([A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}|Apple[[:space:]]*ID|TeamIdentifier=|Receipt[[:space:]]*ID|receipt[[:space:]]*id|private article|私人文章|私密文章)'; then
    fail "$label contains private account or content details"
  fi
}
...
if [[ -z "$ITEM_ID" && "$EXECUTE" == "0" ]]; then
  echo "clean runtime evidence recorder: ready"
...
if [[ "$EXECUTE" != "1" ]]; then
  echo "clean runtime evidence recorder: dry-run"
  echo "- item: $ITEM_ID"
  echo "- title: $TITLE"
  echo "- evidence file: ${EVIDENCE_FILE#$ROOT_DIR/}"
  echo "- execute: pass --execute to write the completed runtime evidence item"
  exit 0
fi
...
echo "clean runtime evidence recorder: recorded $ITEM_ID"
