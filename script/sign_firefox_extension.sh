...
python3 "$ROOT_DIR/script/firefox_extension_release.py" lint
...
python3 "$ROOT_DIR/script/firefox_extension_release.py" install-signed \
  --signed-xpi "${SIGNED_RESULTS[0]}" \
...
echo "After both files are uploaded, verify the published release with:"
echo "  python3 script/firefox_extension_release.py verify-remote"
