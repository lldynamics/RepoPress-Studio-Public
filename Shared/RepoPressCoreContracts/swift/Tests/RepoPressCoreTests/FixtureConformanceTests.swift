...
private enum FixtureAdapter {
  ...
  static func front(_ raw: JSONValue) throws -> FrontRequest {
    ...
    where documentObject[key]?.isNull == true {
...
private func verifyFrontMatter(_ fixture: FixtureCase) throws {
  ...
  let renderer = FrontMatterDocumentRenderer()
  let actual = request.operation == "render"
    ? renderer.render(request.document)
    : renderer.markdownDocument(request.document)
