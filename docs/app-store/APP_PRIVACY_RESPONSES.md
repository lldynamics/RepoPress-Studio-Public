...
- Optional AI requests are sent directly from the Mac to the provider endpoint selected by the user, only after per-endpoint explicit consent; the developer does not proxy or receive API keys, requests, or responses.
- Browser captures travel only between the embedded Safari Web Extension or independently installed Chrome extension and the app through the authenticated `127.0.0.1:17843` loopback endpoint; the developer does not receive them. Edge and Firefox are not supported in this release.
...
| User-configured AI | OpenAI-compatible, DeepSeek, OpenRouter, local model, or custom endpoint selected by the user | Prompt; selected article/site context; selected knowledge excerpts; conversation context; user-selected images; user-owned API key | Explicit consent for that endpoint, followed by a user-initiated AI action |
| Browser capture | RepoPress on the same Mac through `127.0.0.1:17843` | User-confirmed page URL, metadata, selected or extracted content, optional archive, folder choice, and AI-retrieval preference | Extension action, context menu, shortcut, or confirmed batch task |
...
- Per-endpoint AI consent choices are stored locally. AI requests are not metered as an app-defined Pro quota.
- Knowledge-library sources, chunks, and indexes are stored locally.
- Each browser extension, including Safari, stores preferences, limited receipts, an expiring pairing token, and a bounded offline queue in browser-local extension storage.
