...
- Optional GitHub, GitLab, and deployment requests are initiated by the user and sent directly to the selected service; the developer does not retain or access their payloads.
...
- Per-endpoint AI consent choices are stored locally. AI requests are not metered or sold by the app.
- Knowledge-library sources, chunks, and indexes are stored locally.
- The app-side browser connection token is stored only in macOS Keychain. Each browser extension stores its copy of the expiring pairing token, preferences, limited receipts, and bounded offline queue in browser-local extension storage.
- Quick Hide covers the workspace on demand. It is an interface-masking action, not Touch ID/password authentication or local-data encryption. Private-content masking redacts marked content in lists, search, overview, and release-facing surfaces.
...
- Verify provider configuration, API-key entry, AI chat, and AI writing actions are available in the submitted build.
