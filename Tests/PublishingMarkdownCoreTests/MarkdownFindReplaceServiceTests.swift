...
final class MarkdownFindReplaceServiceTests: XCTestCase {
  ...
  func testMatchesHonorsCancellationBeforeScanning() {
    let service = MarkdownFindReplaceService()

    XCTAssertThrowsError(
      try service.matches(
        in: String(repeating: "content ", count: 10_000),
        query: "content",
        shouldCancel: { true }
      )
    ) { error in
  ...
  func testMatchesHonorsCancellationWhileEnumeratingResults() {
    let service = MarkdownFindReplaceService()
    let cancellation = MarkdownFindCancellationCounter(cancelAfter: 4)
...
private final class MarkdownFindCancellationCounter: @unchecked Sendable {
