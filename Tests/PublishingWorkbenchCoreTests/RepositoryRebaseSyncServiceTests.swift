...
final class RepositoryRebaseSyncServiceTests: XCTestCase {
  private final class RecoveryPhaseRecorder: @unchecked Sendable {
    private let lock = NSLock()
    private var storedPhases: [RepositoryRebaseRecoveryContext.Phase] = []

    func record(_ context: RepositoryRebaseRecoveryContext) {
      lock.withLock { storedPhases.append(context.phase) }
    }

    var phases: [RepositoryRebaseRecoveryContext.Phase] {
  ...
  func testPersistsCrashSafeRecoveryPhasesAroundRebaseAndStashApply() throws {
    let fixture = try makeFixture()
    defer { try? FileManager.default.removeItem(at: fixture.base) }
    try commit("local commit\n", path: "local.md", message: "local", at: fixture.local)
    try commit("remote commit\n", path: "remote.md", message: "remote", at: fixture.peer)
    ...
    let recorder = RecoveryPhaseRecorder()
    let service = RepositoryRebaseSyncService { recorder.record($0) }
    let confirmation = try confirmation(from: service.prepare(profile: fixture.profile))

    _ = try service.apply(profile: fixture.profile, confirmation: confirmation)

    XCTAssertEqual(
      recorder.phases,
      [.stashedBeforeRebase, .rebaseCompleted, .stashRestoreInProgress, .completed]
    )
  }

  func testRecoveryRecorderFailureStopsBeforeRebaseAndKeepsVerifiedStash() throws {
    ...
    let service = RepositoryRebaseSyncService { _ in throw ExpectedFailure() }
    ...
    XCTAssertThrowsError(try service.apply(profile: fixture.profile, confirmation: confirmation)) {
      guard case .partial(let recovery, let message) = $0 as? RepositoryRebaseSyncError else {
        return XCTFail("Unexpected error: \($0)")
      }
      XCTAssertEqual(recovery?.phase, .stashedBeforeRebase)
      XCTAssertTrue(message.contains("已停止后续 Git 操作"))
    }
    XCTAssertEqual(try git(["rev-parse", "HEAD"], at: fixture.local), originalHead)
    XCTAssertFalse(
      FileManager.default.fileExists(
        atPath: fixture.local.appendingPathComponent(".git/rebase-merge").path
      )
    )
    XCTAssertFalse(try git(["rev-parse", "refs/stash"], at: fixture.local).isEmpty)
  }
  ...
  func testRebaseConflictLeavesSequencerAndRecoveryStashIntact() throws {
    ...
    XCTAssertThrowsError(try service.apply(profile: fixture.profile, confirmation: confirmation)) {
      guard case .rebaseConflict(let recovery, let paths) = $0 as? RepositoryRebaseSyncError
      else { return XCTFail("Unexpected error: \($0)") }
      recoveryStash = recovery?.stashCommitSHA
  ...
  func testStashRestoreConflictRetainsFrozenRecoveryStash() throws {
    let fixture = try makeFixture()
    defer { try? FileManager.default.removeItem(at: fixture.base) }
    try commit("local commit\n", path: "local.md", message: "local", at: fixture.local)
    try commit("remote version\n", path: "base.md", message: "remote", at: fixture.peer)
    _ = try git(["push", "origin", "main"], at: fixture.peer)
    try write("local WIP\n", to: fixture.local, path: "base.md")
    ...
    var recoveryStash: String?
    XCTAssertThrowsError(try service.apply(profile: fixture.profile, confirmation: confirmation)) {
      guard case .stashRestoreConflict(let recovery, let paths) = $0 as? RepositoryRebaseSyncError
      else { return XCTFail("Unexpected error: \($0)") }
      recoveryStash = recovery.stashCommitSHA
      XCTAssertEqual(paths, ["base.md"])
    }
  ...
  func testRestoresExactStashAfterUserAbortsRebaseConflict() throws {
    ...
    try commit("remote version\n", path: "base.md", message: "remote conflict", at: fixture.peer)
    _ = try git(["push", "origin", "main"], at: fixture.peer)
    try write("recover after abort\n", to: fixture.local, path: "draft.md")

    let service = RepositoryRebaseSyncService()
    let confirmation = try self.confirmation(from: service.prepare(profile: fixture.profile))
    var recovery: RepositoryRebaseRecoveryContext?
    XCTAssertThrowsError(try service.apply(profile: fixture.profile, confirmation: confirmation)) {
      guard case .rebaseConflict(let value, _) = $0 as? RepositoryRebaseSyncError else {
        return XCTFail("Unexpected error: \($0)")
      }
      recovery = value
    }
    let context = try XCTUnwrap(recovery)
    XCTAssertEqual(context.phase, .rebaseConflict)
    _ = try git(["rebase", "--abort"], at: fixture.local)

    let wrongRoot = RepositoryRebaseRecoveryContext(
      repositoryRoot: fixture.base.path,
      gitCommonDirectory: context.gitCommonDirectory,
      branch: context.branch,
      originalHeadSHA: context.originalHeadSHA,
      reviewedRemoteHeadSHA: context.reviewedRemoteHeadSHA,
      stashCommitSHA: context.stashCommitSHA,
      phase: context.phase,
    ...
    XCTAssertEqual(try git(["rev-parse", "refs/stash"], at: fixture.local), context.stashCommitSHA)

    try write("newer stash\n", to: fixture.local, path: "external.md")
    _ = try git(["stash", "push", "--include-untracked", "-m", "external"], at: fixture.local)
    let newerStash = try git(["rev-parse", "refs/stash"], at: fixture.local)

    let result = try service.restoreAfterRebase(profile: fixture.profile, recovery: context)

    XCTAssertTrue(result.stashWasRetained)
    XCTAssertEqual(try content(fixture.local, path: "draft.md"), "recover after abort\n")
    XCTAssertEqual(try git(["rev-parse", "refs/stash"], at: fixture.local), newerStash)
  }
  ...
  func testReportsConflictEnumerationFailureWithoutDiscardingRecoveryStash() throws {
    ...
    var recoveryStash: String?
    XCTAssertThrowsError(try service.apply(profile: fixture.profile, confirmation: confirmation)) {
      guard case .partial(let recovery, let message) = $0 as? RepositoryRebaseSyncError
      else { return XCTFail("Unexpected error: \($0)") }
      recoveryStash = recovery?.stashCommitSHA
