...
final class AIPublishingChatConversationPresentationTests: XCTestCase {
  ...
  func testArchivedConversationPresentationIncludesMobileStyleConversationDetails() {
    let conversation = AIPublishingChatArchivedConversation(
      id: UUID(uuidString: "00000000-0000-0000-0000-000000000123")!,
      title: "发布前问答",
      messages: [
        AIPublishingChatMessage(role: .user, content: "帮我检查发布风险"),
        AIPublishingChatMessage(role: .assistant, content: "建议补充摘要。"),
      ],
      contextMode: .site,
    ...
    XCTAssertEqual(presentation.id, conversation.id)
    XCTAssertEqual(presentation.title, "发布前问答")
    XCTAssertEqual(presentation.messageCountText, "2 条消息")
    XCTAssertEqual(presentation.contextText, "站点上下文")
    XCTAssertEqual(presentation.modelText, "高质量 · deepseek-v4-pro")
    XCTAssertEqual(presentation.subtitle, "2 条消息 · 站点上下文 · 高质量 · deepseek-v4-pro")
  }

  func testArchivedConversationPresentationFallsBackFromPlaceholderTitleToFirstUserMessage() {
    let conversation = AIPublishingChatArchivedConversation(
      title: "New Chat",
      messages: [
        AIPublishingChatMessage(
    ...
    XCTAssertEqual(presentation.title, "已附加图片：diagram.png")
    XCTAssertEqual(presentation.contextText, "通用聊天")
    XCTAssertEqual(presentation.modelText, "自定义 · vision-model")
  }
