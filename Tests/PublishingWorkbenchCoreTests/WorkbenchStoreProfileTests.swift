...
final class WorkbenchStoreProfileTests: XCTestCase {
  func testStartupMigratesLegacyCodexSentinelAndPreservesConfigFields() throws {
    ...
    var malformedConfig = AIProviderConfig(
      preset: .custom,
      baseURL: AIProviderPreset.codexAppServer.defaultBaseURL,
      model: "codex-migration-model",
    ...
    let evidenceKey = AIProviderCapabilityCacheKey(config: malformedConfig)
    let observedAt = Date(timeIntervalSince1970: 1_000)
    malformedConfig.capabilityProbeEvidence = [
      .chat: AIProviderCapabilityProbeEvidence(
        key: evidenceKey,
        capability: .chat,
        outcome: .supported,
        observedAt: observedAt,
        expiresAt: observedAt.addingTimeInterval(3_600),
        statusCode: 200,
        detail: "保留探测证据"
      )
    ]
    let profile = SiteProfile(
      id: profileID,
      name: "迁移站点",
      aiConnectionProfileID: connectionID,
      aiProviderConfig: malformedConfig
    )
    ...
    let snapshot = WorkbenchSnapshot(
      profiles: [profile],
      aiConnectionProfiles: [connection],
      activeProfileID: profileID,
      drafts: [ArticleDraft.empty(profile: profile)],
      releaseRecords: []
    )
    let originalPersistence = Data("preloaded snapshot stays untouched".utf8)
    try originalPersistence.write(to: persistenceURL)
    ...
    let migratedConnection = try XCTUnwrap(store.aiConnectionProfiles.first)
    XCTAssertEqual(migratedConnection.config.preset, .codexAppServer)
    XCTAssertEqual(migratedConnection.config.baseURL, malformedConfig.baseURL)
    XCTAssertEqual(migratedConnection.config.model, malformedConfig.model)
    XCTAssertEqual(migratedConnection.config.requiresAPIKey, false)
    XCTAssertEqual(migratedConnection.config.advancedSettings, malformedConfig.advancedSettings)
    XCTAssertEqual(
      migratedConnection.config.capabilityProbeEvidence,
      malformedConfig.capabilityProbeEvidence
    )
    XCTAssertEqual(store.activeProfile.aiProviderConfig, migratedConnection.config)
    XCTAssertTrue(store.hasUnsavedChanges)
    XCTAssertEqual(try Data(contentsOf: persistenceURL), originalPersistence)

    let normalizedSnapshot = persistence.snapshot(from: store)
    let secondStore = WorkbenchStore(
      persistence: persistence,
      initialSnapshotSource: .preloaded(
        WorkbenchSnapshotLoadResult(snapshot: normalizedSnapshot)
      ),
      safeMode: true
    )
    XCTAssertFalse(secondStore.hasUnsavedChanges)
  }
