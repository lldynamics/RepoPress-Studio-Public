...
final class RepositorySafeSyncServiceTests: XCTestCase {
  ...
  func testFastForwardsCleanBehindRepository() throws {
    let fixture = try makeFixture()
    defer { try? FileManager.default.removeItem(at: fixture.base) }
    try advanceRemote(fixture, path: "remote.md", contents: "remote\n")
    ...
    XCTAssertEqual(result.preservedLocalPaths, [])
    XCTAssertEqual(result.reconciledCollisionPaths, [])
    XCTAssertNil(result.recoveryArchiveURL)
    XCTAssertEqual(try content(fixture.worktree, path: "remote.md"), "remote\n")
    XCTAssertEqual(
      try git(["status", "--porcelain=v1"], at: fixture.worktree).trimmedForPublishing,
      ""
    )
  }

  func testRejectsDivergedRepositoryWithoutStartingFastForward() throws {
    let fixture = try makeFixture()
    defer { try? FileManager.default.removeItem(at: fixture.base) }
    try write("local\n", to: fixture.worktree, path: "local.md")
    _ = try git(["add", "--", "local.md"], at: fixture.worktree)
    _ = try git(["commit", "-m", "local"], at: fixture.worktree)
    try advanceRemote(fixture, path: "remote.md", contents: "remote\n")
