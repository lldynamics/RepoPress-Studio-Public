...
final class RPNoteCloudPayloadTests: XCTestCase {
  ...
  func testAttachmentPlanUploadsOnlyChangedAttachmentsAndDeletesRemovedOnes() {
    let noteID = UUID()
    let kept = RPNoteAttachment(fileName: "kept", mimeType: "x/a", data: Data([1]))
    ...
    let plan = RPNoteCloudAttachmentPlan.plan(
      for: .upsert(note: note, revision: "r", systemFields: nil), baselines: baselines
    )
    XCTAssertEqual(plan.uploads.map(\.recordName), [name(edited.id)])
