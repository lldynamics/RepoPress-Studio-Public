...
final class RPNotesPackageCodecTests: XCTestCase {
    ...
    func testManifestEncodingIsStableAndUsesDeclaredPaths() throws {
        ...
        let object = try JSONSerialization.jsonObject(with: manifestData(first)) as! [String: Any]
        ...
        XCTAssertEqual(attachment["path"] as? String, "attachments/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb")
    }
    ...
    func testRoundTripsSubMillisecondDates() throws {
        ...
        let package = RPNotePackage(createdAt: createdAt, notes: [
            RPNote(id: UUID(uuidString: "33333333-3333-3333-3333-333333333333")!, createdAt: createdAt, updatedAt: updatedAt, markdown: "body")
        ])
    ...
    func testRejectsUndeclaredAttachmentAndWrongSize() throws {
        ...
        var attachments = note["attachments"] as! [[String: Any]]
        attachments[0]["byteCount"] = 4
        note["attachments"] = attachments
        manifest["notes"] = [note]
        let wrapper = wrapper(manifest: manifest, body: body, attachment: attachment)
        assertError(.integrityMismatch("attachments/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb")) {
            _ = try RPNotesPackageCodec.decode(wrapper)
        }
    }
    ...
    private func fixturePackage() -> RPNotePackage {
        ...
        return RPNotePackage(createdAt: date, notes: [
            RPNote(
                ...
                attachments: [RPNoteAttachment(id: UUID(uuidString: "BBBBBBBB-BBBB-BBBB-BBBB-BBBBBBBBBBBB")!, fileName: "a.bin", mimeType: "application/octet-stream", data: Data([1, 2, 3]))]
            )
        ])
    }
    ...
    private func fixtureNote(body: Data = Data("body".utf8), attachment: Data? = nil) -> [String: Any] {
        ...
        if let attachment {
            result["attachments"] = [[
                ...
                "byteCount": attachment.count, "path": "attachments/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb", "sha256": sha(attachment)
            ]]
        }
        return result
    }
