...
final class RPNotesPackageCodecTests: XCTestCase {
    ...
    let package = RPNotePackage(
      ...
      notes: [
        RPNote(
          ...
          attachments: [
            RPNoteAttachment(
              id: attachmentID, fileName: "截图.png", mimeType: "image/png",
              data: Data([0, 1, 2, 255]))
          ]
        )
      ])
  ...
  func testManifestEncodingIsStableAndUsesDeclaredPaths() throws {
    ...
    let object = try XCTUnwrap(
      try JSONSerialization.jsonObject(with: manifestData(first)) as? [String: Any])
    ...
    XCTAssertEqual(
      ...
      "attachments/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb")
  }
  ...
  func testRoundTripsSubMillisecondDates() throws {
    ...
    let package = RPNotePackage(
      ...
      notes: [
        RPNote(
          id: UUID(uuidString: "33333333-3333-3333-3333-333333333333")!, createdAt: createdAt,
          updatedAt: updatedAt, markdown: "body")
      ])
  ...
  func testRejectsTamperedMarkdownBeforeReturningNotes() throws {
    let wrapper = wrapper(
      manifest: fixtureManifest(body: Data("body".utf8)), body: Data("changed".utf8))
  ...
  func testRejectsSymbolicLinkInDeclaredBodyPath() throws {
    ...
    let link = FileWrapper(
      symbolicLinkWithDestinationURL: URL(fileURLWithPath: "/private/tmp/elsewhere.md"))
    let wrapper = FileWrapper(directoryWithFileWrappers: [
      "manifest.json": FileWrapper(
        regularFileWithContents: try JSONSerialization.data(withJSONObject: manifest)),
      "notes": FileWrapper(directoryWithFileWrappers: [
        "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa.md": link
      ]),
  ...
  private func fixturePackage() -> RPNotePackage {
    ...
    return RPNotePackage(
      createdAt: date,
      notes: [
        RPNote(
          id: UUID(uuidString: "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA")!, title: "Fixture",
          tags: ["tag"],
          createdAt: date, updatedAt: date, markdown: "body",
          attachments: [
            RPNoteAttachment(
              id: UUID(uuidString: "BBBBBBBB-BBBB-BBBB-BBBB-BBBBBBBBBBBB")!, fileName: "a.bin",
              mimeType: "application/octet-stream", data: Data([1, 2, 3]))
          ]
        )
      ])
  }

  private func fixtureWrapper(body: Data = Data("body".utf8), bodyPath: String? = nil)
    -> FileWrapper
  {
  ...
  private func fixtureManifest(body: Data = Data("body".utf8), attachment: Data? = nil) -> [String:
    Any]
  {
    [
  ...
  private func fixtureNote(body: Data = Data("body".utf8), attachment: Data? = nil) -> [String: Any]
  {
    ...
    if let attachment {
      result["attachments"] = [
        [
          "id": "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb", "fileName": "a.bin",
          "mimeType": "application/octet-stream",
          "byteCount": attachment.count,
          "path":
            "attachments/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb",
