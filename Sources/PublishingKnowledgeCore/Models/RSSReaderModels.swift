...
public struct RSSFeed: Codable, Hashable, Identifiable, Sendable {
  ...
  public init(from decoder: Decoder) throws {
    ...
    self.init(
      ...
      lastIssue: decodedLastIssue
        ?? decodedLastError.map {
          RSSFeedIssue(
            stage: .transport,
            category: .unknown,
            retryStrategy: .automatic,
            ...
            occurredAt: (try? container.decodeIfPresent(Date.self, forKey: .lastRefreshAttemptAt))
              ?? Date()
          )
        },
      lastRefreshAttemptAt: try container.decodeIfPresent(Date.self, forKey: .lastRefreshAttemptAt),
      refreshFailureCount: try container.decodeIfPresent(Int.self, forKey: .refreshFailureCount)
        ?? 0,
      nextRetryAt: try container.decodeIfPresent(Date.self, forKey: .nextRetryAt),
      lastRefreshDuration: try container.decodeIfPresent(
        TimeInterval.self, forKey: .lastRefreshDuration)
    )
  }
}
...
public struct RSSArticleHeader: Codable, Hashable, Identifiable, Sendable {
  ...
  public init(article: RSSArticle) {
    let previewHTML =
      article.summaryHTML.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
      ? article.contentHTML
      : article.summaryHTML
    let previewSource =
      previewHTML.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
...
extension RSSArticle {
  ...
  public init(header: RSSArticleHeader) {
    self.init(
      id: header.id,
  ...
  public mutating func apply(header: RSSArticleHeader) {
    title = header.title
    link = header.link
    coverURL = header.coverURL
    author = header.author
