...
extension KnowledgeDatabase {
  ...
  /// Only wrap writes confined to folder labels. Advance a *current* cache
  /// token past those writes without hiding an earlier document/vector change.
  /// All callers hold the database lock; failures retain conservative invalidation.
  func preservingSemanticIndexForFolderLabelWriteUnlocked<Value>(
    _ operation: () throws -> Value
  ) rethrows -> Value {
    let wasCurrent = semanticFlatVectorIndexChangeToken == Int64(sqlite3_total_changes(handle))
  ...
  private func semanticFlatVectorIndex(
    ...
    return try withCachedStatementUnlocked(
      ...
      semanticFlatVectorIndexBuildCount &+= 1
      semanticFlatVectorIndexChangeToken = Int64(sqlite3_total_changes(handle))
      return index
    }
  }
