...
extension RSSReaderDatabase {
  ...
  func upsertFullTextRecordIfCurrentSource(
    ...
    try withLock {
      try transactionUnlocked {
        guard try articleLinkUnlocked(articleID: record.articleID) == sourceURL.absoluteString
        else { return }
        try upsertFullTextRecordUnlocked(record)
        didPersist = true
      }
  ...
  private func decodeFullTextRecord(_ statement: OpaquePointer?) throws -> RSSArticleFullTextRecord
  {
    ...
    return RSSArticleFullTextRecord(
      ...
      extractorIdentifier: text(statement, 6)
        ?? RSSArticleFullTextRecord.defaultExtractorIdentifier,
