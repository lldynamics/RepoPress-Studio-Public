...
extension KnowledgeDatabase {
  ...
  func createFolder(name: String) throws -> KnowledgeFolder {
    try withLock {
      ...
      return try preservingSemanticIndexForFolderLabelWriteUnlocked {
  ...
  func renameFolder(id: UUID, name: String) throws -> KnowledgeFolder {
    try withLock {
      ...
      let createdAt = try withCachedStatementUnlocked(
        "SELECT created_at FROM knowledge_folders WHERE id = ? LIMIT 1;"
      ) { statement in
        bind(id.uuidString, at: 1, to: statement)
  ...
  func deleteFolder(id: UUID) throws {
    try withLock {
      ...
      do {
        try withCachedStatementUnlocked(
          "UPDATE knowledge_documents SET folder_id = NULL WHERE folder_id = ?;"
        ) { statement in
          bind(id.uuidString, at: 1, to: statement)
  ...
  func setSourceByteCount(_ sourceByteCount: Int64, documentID: UUID) throws {
    try withLock {
      try withCachedStatementUnlocked(
        "UPDATE knowledge_documents SET source_byte_count = ? WHERE id = ?;"
      ) { statement in
