...
extension KnowledgeStore {
    ...
    while true {
      ...
      do {
        ...
        if !searchText.trimmedForPublishing.isEmpty {
          ...
          // Search hits embed their document value. Reloading the library must
          // also refresh the reading projection for the still-valid selection;
          // otherwise the inspector can keep showing the prior revision while
          // the new search is running.
          if let nextSelection, documents.contains(where: { $0.id == nextSelection }) {
  ...
  public func selectSearchResult(_ result: KnowledgeSearchResult) {
    guard let document = documents.first(where: { $0.id == result.document.id }) else {
      selectedSearchResult = nil
      selectedResultQuery = ""
      statusMessage = "资料库中找不到要打开的资料。"
      return
    }
    guard document.currentRevisionID == result.chunk.revisionID else {
      selectedSearchResult = nil
      selectedResultQuery = ""
      selectDocument(document.id)
      statusMessage = CoreL10n.text("资料已更新，已打开最新版本；请重新搜索以定位片段。")
      return
    }
    guard
      searchText.trimmedForPublishing.isEmpty
        || visibleSearchResults.contains(where: { $0.id == result.id })
    else {
      selectedSearchResult = nil
      selectedResultQuery = ""
      selectDocument(document.id)
  ...
  /// An explicit cross-workspace navigation reveals the document without
  /// launching another search that could replace the selected hit later.
  @discardableResult
  public func revealSearchResult(_ result: KnowledgeSearchResult, query: String) -> Bool {
    guard let document = documents.first(where: { $0.id == result.document.id }) else {
      statusMessage = CoreL10n.text("资料库中找不到要打开的资料。")
      return false
    }
    updateSearchText("")
    folderScope = .all
