...
enum RSSRefreshWorker {
  static func run(
    ...
    do {
      ...
      if let database {
        guard
          let persistedArticles = try database.upsertFeedAndArticlesIfURLMatches(
            updatedFeed,
  ...
  func apply(_ fetchOutcome: RefreshFetchOutcome) async -> RefreshOutcome {
    ...
    if workerResult.succeeded {
      ...
      if let mergeResult = workerResult.mergeResult {
        ...
        updatedHeaders =
          currentHeaders.filter { $0.feedID != feed.id }
          + rebasedFeedHeaders
      ...
      do {
        ...
        return RefreshOutcome(
          ...
          skipped: false,
          message:
            "\(workerResult.updatedFeed.displayTitle)：无法保存刷新结果：\(error.localizedDescription)",
    ...
    if let persistenceError = workerResult.persistenceError {
      message =
        "\(workerResult.updatedFeed.displayTitle)：\(issue.userMessage)；同时无法保存错误状态：\(persistenceError)"
    } else {
