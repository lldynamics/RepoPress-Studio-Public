...
extension RSSReaderDatabase {
  ...
  func withCancellableLock<T>(_ body: () throws -> T) throws -> T {
    ...
    while !lock.lock(before: Date(timeIntervalSinceNow: 0.02)) {
      queryHooks.didWaitForCancellableLock?()
      try Task.checkCancellation()
    }
    defer { lock.unlock() }
    try Task.checkCancellation()
  ...
  func withCancellationProgressHandler<T>(_ body: () throws -> T) throws -> T {
    try Task.checkCancellation()
    guard let handle else { throw databaseErrorUnlocked() }
    // Let SQLite abort a long FTS scan or ORDER BY before sqlite3_step returns a row.
    let contextPointer: UnsafeMutableRawPointer?
    if let didInvokeProgressHandler = queryHooks.didInvokeProgressHandler {
      contextPointer = Unmanaged.passRetained(
        RSSSQLiteCancellationProgressContext(
          didInvokeProgressHandler: didInvokeProgressHandler
        )
      ).toOpaque()
    } else {
      contextPointer = nil
    ...
    sqlite3_progress_handler(
      handle,
      1_000,
      rssSQLiteCancellationProgressHandler,
      contextPointer
    )
    defer {
      sqlite3_progress_handler(handle, 0, nil, nil)
      if let contextPointer {
        Unmanaged<RSSSQLiteCancellationProgressContext>.fromOpaque(contextPointer).release()
      }
    }
  ...
  func withCachedStatementUnlocked<T>(_ sql: String, _ body: (OpaquePointer) throws -> T) throws
    -> T
  {
