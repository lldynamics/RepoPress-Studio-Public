...
public final class KnowledgeStore: ObservableObject {
  ...
  @Published public internal(set) var pinnedDocumentIDs: Set<UUID> = [] {
    didSet {
      invalidateListPresentation()
      if pinnedDocumentIDs != oldValue { refreshSearchAfterKnowledgeMutation() }
    }
  }
  ...
  func recordKnowledgeImportEvent(
    outcome: KnowledgeImportEvent.Outcome,
  ...
  func knowledgeImportOutcome(for result: KnowledgeImportResult) -> KnowledgeImportEvent.Outcome {
  ...
  /// Every accepted persistence mutation advances the generation immediately,
  /// so an in-flight search may finish before the mutation's projection work
  /// has returned to its caller. Re-run the active query after the mutation
  /// completes (including failure) to avoid leaving `isSearching` stuck or
  /// retaining a result snapshot from before the mutation.
  func refreshSearchAfterKnowledgeMutation() {
    guard !searchText.trimmedForPublishing.isEmpty else {
      searchTask?.cancel()
      isSearching = false
      return
    }
    // The documents/pins observers repeat this after the authoritative
    // projection is installed. Never infer that ordering from Task.yield().
