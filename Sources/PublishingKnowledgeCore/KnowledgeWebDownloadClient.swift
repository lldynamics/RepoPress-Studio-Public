...
package enum KnowledgeWebDownloadError: LocalizedError, Hashable, Sendable {
  case invalidURL
...
package enum KnowledgeResolvedAddress: Hashable, Sendable {
  case ipv4([UInt8])
  case ipv6([UInt8])

  package init?(presentation: String) {
    var ipv4 = in_addr()
  ...
  package var isPubliclyRoutable: Bool {
    switch self {
    case .ipv4(let bytes):
...
package enum KnowledgeWebDownloadPolicy {
  package typealias Resolver = @Sendable (String) throws -> [KnowledgeResolvedAddress]

  package static let defaultResolver: Resolver = { host in
    try resolveAddresses(host)
  }

  package struct ValidatedEndpoint: Hashable, Sendable {
    package var url: URL
    package var addresses: [KnowledgeResolvedAddress]
  }

  package static func validatedURL(
    _ url: URL,
  ...
  package static func validatedEndpoint(
    _ url: URL,
    resolver: Resolver = defaultResolver
  ...
  package static func resolveAddresses(_ host: String) throws -> [KnowledgeResolvedAddress] {
...
package struct KnowledgeWebDownloadResponse: Sendable {
  package var data: Data
  package var response: HTTPURLResponse
...
package struct KnowledgePinnedHTTPResponse: Sendable {
  package var data: Data
  package var statusCode: Int
  package var headerFields: [String: String]

  package init(data: Data, statusCode: Int, headerFields: [String: String]) {
    self.data = data
...
package enum KnowledgeHTTP1ResponseParser {
...
package enum KnowledgePinnedHTTPSClient {
  ...
  package static func encodedRequest(_ request: URLRequest, url: URL) throws -> Data {
    guard let components = URLComponents(url: url, resolvingAgainstBaseURL: false),
...
package struct KnowledgeWebDownloadClient: Sendable {
