...
extension RSSArticleDOMExtractionService {
  ...
  fileprivate static let maximumHTMLByteCount = 4 * 1024 * 1024
  fileprivate static let maximumDOMNodeCount = 100_000
  fileprivate static let maximumDOMDepth = 256
  fileprivate static let maximumAnalysisWork = 750_000
  fileprivate static let maximumCandidateCount = 2_048
  fileprivate static let encodingProbeByteCount = 32 * 1024

  fileprivate final class AnalysisBudget {
  ...
  fileprivate struct CandidateStats {
  ...
  fileprivate enum DetectedEncoding {
    case ascii
    case utf8
    case utf16
    case isoLatin1
  ...
  fileprivate struct EncodingCandidate {
    let encoding: DetectedEncoding
    let score: Double
    let order: Int
  }

  fileprivate static let dangerousElementNames: Set<String> = [
  ...
  fileprivate static let semanticMarkerAttribute = "data-rss-original-tag"
  fileprivate static let semanticHTMLTags = [
    "article", "main", "section", "nav", "aside", "footer", "header",
  ]

  fileprivate static let negativeHintFragments: [String] = [
  ...
  fileprivate static let positiveHintFragments: [String] = [
    "article", "body", "content", "entry", "main", "markdown", "post", "prose",
    "richtext", "story", "text",
  ]

  fileprivate static let shellFragments: [String] = [
  ...
  fileprivate func decodeHTML(_ data: Data, textEncodingName: String?) throws -> String {
  ...
  fileprivate func decodeUndeclaredHTML(_ data: Data) throws -> String {
  ...
  fileprivate func encodingProbeText(in data: Data, encoding: DetectedEncoding) -> String? {
  ...
  fileprivate func decodeProbeSegment(_ data: Data, encoding: String.Encoding) -> String? {
  ...
  fileprivate func encodingPlausibilityScore(_ value: String, encoding: DetectedEncoding) -> Double
  {
  ...
  fileprivate func isHan(_ scalar: UnicodeScalar) -> Bool {
  ...
  fileprivate func isJapaneseKana(_ scalar: UnicodeScalar) -> Bool {
    let value = scalar.value
  ...
  fileprivate func declaredCharset(in data: Data) -> String? {
    ...
    for pattern in patterns {
      ...
      guard let match = expression.firstMatch(in: probe, range: range),
        let valueRange = Range(match.range(at: 1), in: probe)
      else { continue }
  ...
  fileprivate func encoding(for rawName: String) -> DetectedEncoding? {
    let normalized =
      rawName
      .lowercased()
  ...
  fileprivate func looksLikeHTML(_ value: String) -> Bool {
    let lowercased = value.lowercased()
  ...
  fileprivate func normalizedHTMLForDOM(_ html: String) -> String {
    var normalized = html
  ...
  fileprivate func validateDOMShape(_ root: XMLElement) throws {
    var nodeCount = 0
  ...
  fileprivate func removeNoise(from element: XMLElement, budget: AnalysisBudget) throws {
    for child in element.children ?? [] {
      try budget.consume()
  ...
  fileprivate func remove(_ child: XMLNode, from parent: XMLElement) {
    guard let index = parent.children?.firstIndex(where: { $0 === child }) else { return }
    parent.removeChild(at: index)
  }

  fileprivate func shouldRemove(_ element: XMLElement, name: String) -> Bool {
  ...
  fileprivate func collectCandidates(in body: XMLElement, budget: AnalysisBudget) throws
    -> [Candidate]
  {
  ...
  fileprivate func chooseCandidate(
  ...
  fileprivate func candidateScore(_ candidate: Candidate, expectedTitle: String?) -> Double {
    ...
    var score =
      0.04 + lengthScore + paragraphScore + punctuationScore + structureScore
  ...
  fileprivate func confidenceScore(for candidate: Candidate, titleSimilarity: Double?) -> Double {
  ...
  fileprivate func stats(for element: XMLElement, budget: AnalysisBudget) throws -> CandidateStats {
  ...
  fileprivate func extractTitle(from root: XMLElement, candidate: XMLElement) -> String? {
  ...
  fileprivate func preferredTitle(
  ...
  fileprivate func firstMetadataTitle(in root: XMLElement) -> String? {
  ...
  fileprivate func firstMetadataAuthor(in root: XMLElement) -> String? {
  ...
  fileprivate func extractAuthor(from root: XMLElement, candidate: XMLElement) -> String? {
    for name in ["author", "article:author", "byl"] {
      if let value = firstMetaValue(named: name, in: root) {
        return value
      }
  ...
  fileprivate func firstMetaValue(named name: String, in root: XMLElement) -> String? {
    ...
    for element in candidates where elementName(element) == "meta" {
      let key =
        attributeValue("name", on: element)?.lowercased()
        ?? attributeValue("property", on: element)?.lowercased()
      guard key == lowercasedName,
        let content = attributeValue("content", on: element)
      else { continue }
  ...
  fileprivate func firstHeadingText(in element: XMLElement) -> String? {
  ...
  fileprivate func firstDescendant(named name: String, in root: XMLElement) -> XMLElement? {
    let wanted = name.lowercased()
    return firstElement(in: root) { elementName($0) == wanted }
  }

  fileprivate func descendants(of root: XMLElement) -> [XMLNode] {
    var result: [XMLNode] = []
  ...
  fileprivate func textContent(of node: XMLNode) -> String {
    var result = ""
  ...
  fileprivate func serializedHTML(for element: XMLElement) -> String {
  ...
  fileprivate func hintText(for element: XMLElement) -> String {
    [
  ...
  fileprivate func containsNegativeHint(_ hints: String) -> Bool {
  ...
  fileprivate func positiveHintCount(in hints: String) -> Int {
    let tokens = Set(hints.split { !$0.isLetter && !$0.isNumber }.map(String.init))
  ...
  fileprivate func negativeHintCount(in hints: String) -> Int {
    let tokens = Set(hints.split { !$0.isLetter && !$0.isNumber }.map(String.init))
  ...
  fileprivate func attributeValue(_ name: String, on element: XMLElement) -> String? {
    element.attribute(forName: name)?.stringValue?.trimmingCharacters(in: .whitespacesAndNewlines)
  }

  fileprivate func structuralName(of element: XMLElement) -> String {
    attributeValue(Self.semanticMarkerAttribute, on: element)?.lowercased()
      ?? elementName(element)
  }

  fileprivate func elementName(_ element: XMLElement) -> String {
    element.name?.split(separator: ":").last.map(String.init)?.lowercased() ?? ""
  }

  fileprivate func normalizedText(_ source: String) -> String {
    source
  ...
  fileprivate func titleSimilarity(between lhs: String, and rhs: String?) -> Double {
    guard let rhs,
      let left = normalizedTitle(lhs).nilIfEmpty,
      let right = normalizedTitle(rhs).nilIfEmpty
    else { return 0 }
  ...
  fileprivate func normalizedTitle(_ title: String) -> String {
    title
      .lowercased()
      .filter { $0.isLetter || $0.isNumber }
  }

  fileprivate func isObviousShell(
    text: String,
    title: String?,
    expectedTitle: String?,
    ...
    let titleLooksLikeShell =
      title.map { titleValue in
    ...
    return titleLooksLikeShell || (shellHit && (short || linkHeavy || mismatch))
      || (short && linkHeavy)
  }
}
