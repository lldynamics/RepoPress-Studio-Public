...
public enum RSSArticleCoverResolver {
  ...
  public static func firstImageURL(
    ...
    expression.enumerateMatches(in: html, range: range) { match, _, stop in
      ...
      guard let rawValue,
        let url = normalizedURL(rawValue, relativeTo: baseURL)
      else { return }
      result = url
      stop.pointee = true
    }
  ...
  public static func firstSocialImageURL(
    ...
    expression.enumerateMatches(in: html, range: range) { match, _, stop in
      ...
      let tag = String(html[tagRange])
      let property = (attributeValue(in: tag, named: ["property", "name"]) ?? "").lowercased()
    ...
    for name in names {
      guard
        let expression = try? NSRegularExpression(
          pattern:
            "(?is)\\b\(NSRegularExpression.escapedPattern(for: name))\\s*=\\s*(?:\\\"([^\\\"]*)\\\"|'([^']*)'|([^\\s>]+))"
        )
      else { continue }
