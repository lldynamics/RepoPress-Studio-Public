...
final class KnowledgeLibraryBackupService: @unchecked Sendable {
  ...
  private func validatedBackup(
    ...
    guard databaseInspection.userVersion == manifest.databaseUserVersion,
      ...
      databaseInspection.chunkCount == manifest.chunkCount
    else {
  ...
  private func validateFileLimits(
    ...
    for record in records {
      guard record.byteCount >= 0,
        record.byteCount <= limits.maximumSingleFileByteCount
      else {
      ...
      guard !addition.overflow,
        addition.partialValue <= limits.maximumTotalByteCount
      else {
  ...
  private func validatedFileRecord(
    ...
    try withOpenRegularFile(relativePath: relativePath, under: directoryURL) {
  ...
  private func copyValidatedRegularFile(
    ...
    try withOpenRegularFile(relativePath: relativePath, under: sourceRootURL) {
      descriptor, byteCount in
      guard byteCount <= limits.maximumSingleFileByteCount else {
        throw KnowledgeLibraryBackupError.fileTooLarge(
          path: relativePath,
...
private struct KnowledgeLibraryRollbackError: LocalizedError {
  ...
  var errorDescription: String? {
    var description =
      "\(operation)失败：\(primaryError.localizedDescription)；自动回滚失败：\(rollbackError.localizedDescription)。"
    if let recoveryURL {
