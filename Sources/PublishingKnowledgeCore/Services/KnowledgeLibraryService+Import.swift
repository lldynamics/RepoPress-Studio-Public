...
extension KnowledgeLibraryService {
  ...
  func fileCandidate(
    ...
    guard
      let data = try? BoundedFileReader.data(
        at: sourceURL,
        maximumByteCount: (isImage ? 25 : 50) * 1_024 * 1_024
      )
    else {
      throw KnowledgeLibraryError.unreadableSource(sourceURL.path)
    }
    return try candidate(
      ...
      sourceURL: isImage ? nil : sourceURL.standardizedFileURL,
  ...
  func candidate(
    ...
    return KnowledgeImportCandidate(
      ...
      imageMetadata: extraction.imageMetadata,
      originalData: data,
