...
public enum RSSArticleSystemTranslationPlanningService {
  ...
  public static func makePlan(
    ...
    let sourceHTML =
      article.contentHTML.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
      ? article.summaryHTML
      : article.contentHTML
...
private enum RSSArticleHTMLTranslationParser {
  ...
  private static func endOfDelimitedToken(
    ...
    let searchStart = source.index(
      start,
      offsetBy: min(delimiter.count == 3 ? 4 : 9, source.distance(from: start, to: source.endIndex))
    )
