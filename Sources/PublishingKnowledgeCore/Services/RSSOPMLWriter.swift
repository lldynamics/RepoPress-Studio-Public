...
public enum RSSOPMLWriter {
  ...
  private static func applying(
    ...
    guard
      let feedURL = RSSSubscriptionURLPrivacy.redactingSuspectedCredentialQueryValues(
        in: subscription.url
      )
    else {
    ...
    if let originalSiteURL = subscription.siteURL {
      guard
        let redactedSiteURL = RSSSubscriptionURLPrivacy.redactingSuspectedCredentialQueryValues(
          in: originalSiteURL
        )
      else {
  ...
  private static func serialize(
    ...
    for subscription in subscriptions {
      ...
      let htmlURLAttribute =
        subscription.siteURL.map {
