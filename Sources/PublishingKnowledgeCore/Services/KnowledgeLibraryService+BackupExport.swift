...
extension KnowledgeLibraryService {
  ...
  package func createBackupSynchronously(
    at destinationURL: URL,
  ...
  package func inspectBackupSynchronously(at backupURL: URL) throws -> KnowledgeLibraryBackupPreview
  {
    try KnowledgeLibraryBackupService(rootURL: rootURL, fileManager: fileManager)
      .inspectBackup(at: backupURL)
  }

  package static func pendingRestoreURL(for rootURL: URL) -> URL {
    KnowledgeLibraryBackupService.pendingRestoreURL(for: rootURL)
  }
  ...
  func safeStorageFileURL(for reference: String) -> URL? {
    ...
    let rootPathPrefix =
      resolvedRootURL.path.hasSuffix("/")
