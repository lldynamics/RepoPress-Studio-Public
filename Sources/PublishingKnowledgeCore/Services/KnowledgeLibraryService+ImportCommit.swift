...
extension KnowledgeLibraryService {
  ...
  func commitSynchronously(
    ...
    for candidate in preview.candidates {
      ...
      let existingDocument = try candidate.existingDocumentID.flatMap {
        try database.document(id: $0)
      }
      ...
      let document = KnowledgeDocument(
        ...
        sourceByteCount: Int64(
          candidate.originalData?.count ?? candidate.normalizedText.utf8.count),
  ...
  func originalStorageReference(for candidate: KnowledgeImportCandidate) -> String? {
    ...
    return
      "blobs/sha256/\(String(candidate.originalContentHash.prefix(2)))/\(candidate.originalContentHash)\(suffix)"
  }
    ...
    do {
      for reference in artifacts.keys.sorted() {
        ...
        guard let data = artifacts[reference],
          let destinationURL = safeStorageFileURL(for: reference)
        else {
          throw KnowledgeLibraryError.database("非法的内容存储路径：\(reference)")
        }
