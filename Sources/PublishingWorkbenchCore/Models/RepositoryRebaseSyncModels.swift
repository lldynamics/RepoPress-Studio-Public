...
/// The minimum durable evidence required to restore a verified WIP stash after
/// a rebase has completed or the user has explicitly aborted its sequencer.
/// This deliberately contains repository identity rather than a `SiteProfile`
/// so a Store can persist it without serializing credentials or profile state.
public struct RepositoryRebaseRecoveryContext: Codable, Hashable, Sendable, Identifiable {
  public enum Phase: String, Codable, Hashable, Sendable {
    /// The WIP stash has been verified and durably recorded, but Git has not
    /// yet proven whether the rebase started or completed.
    case stashedBeforeRebase
    /// A rebase stopped on conflicts. The caller must finish or abort its
    /// sequencer before an exact stash restore can be attempted.
    case rebaseConflict
    /// Rebase reached a new HEAD and the automatic stash restore is pending.
    case rebaseCompleted
    /// The exact stash is about to be applied. A crash in this phase is
    /// intentionally fail-closed because repeating `stash apply` is unsafe.
    case stashRestoreInProgress
    /// Applying the exact stash conflicted. The stash remains recoverable and
    /// the conflict must be resolved before another restore attempt.
    case stashRestoreConflict
    /// Git returned an inconclusive failure; do not attempt automatic restore.
    case incomplete
    /// The exact stash was already applied. A leftover record is cleanup-only
    /// and must never cause the stash to be applied again.
    case completed
  }
  ...
  public let gitCommonDirectory: String
  public let branch: String
  public let originalHeadSHA: String
  public let reviewedRemoteHeadSHA: String
  public let stashCommitSHA: String
  public let phase: Phase
  public let createdAt: Date

  public var id: String {
    [repositoryRoot, branch, stashCommitSHA, phase.rawValue].joined(separator: "\u{1F}")
  }
  ...
  public init(
    ...
    reviewedRemoteHeadSHA: String,
    stashCommitSHA: String,
    phase: Phase,
    createdAt: Date = Date()
  ) {
    self.repositoryRoot = repositoryRoot
    self.gitCommonDirectory = gitCommonDirectory
    self.branch = branch
    self.originalHeadSHA = originalHeadSHA
    self.reviewedRemoteHeadSHA = reviewedRemoteHeadSHA
    self.stashCommitSHA = stashCommitSHA
    self.phase = phase
    self.createdAt = createdAt
  }

  public func changingPhase(to phase: Phase) -> Self {
    Self(
      repositoryRoot: repositoryRoot,
      gitCommonDirectory: gitCommonDirectory,
      branch: branch,
      originalHeadSHA: originalHeadSHA,
      reviewedRemoteHeadSHA: reviewedRemoteHeadSHA,
      stashCommitSHA: stashCommitSHA,
      phase: phase,
      createdAt: createdAt
    )
  }
...
public struct RepositoryRebaseRecoveryResult: Hashable, Sendable {
  public let branch: String
  public let restoredHeadSHA: String
  public let restoredLocalChanges: [String]
  public let stashWasRetained: Bool

  public init(
    branch: String,
    restoredHeadSHA: String,
    restoredLocalChanges: [String],
    stashWasRetained: Bool
  ) {
    self.branch = branch
    self.restoredHeadSHA = restoredHeadSHA
    self.restoredLocalChanges = restoredLocalChanges
    self.stashWasRetained = stashWasRetained
  }
}
...
public enum RepositoryRebaseSyncError: Error, LocalizedError, Hashable, Sendable {
  ...
  case recoveryStashUnavailable(RepositoryRebaseRecoveryContext)
  case rebaseConflict(recovery: RepositoryRebaseRecoveryContext?, paths: [String])
  case stashRestoreConflict(recovery: RepositoryRebaseRecoveryContext, paths: [String])
  case partial(recovery: RepositoryRebaseRecoveryContext?, message: String)
  ...
  public var errorDescription: String? {
    ...
    case .recoveryStashUnavailable(let recovery):
      "恢复 stash \(recovery.stashCommitSHA.prefix(12)) 不再位于 refs/stash，未执行恢复或删除。"
    case .rebaseConflict(let recovery, let paths):
      "变基遇到冲突：\(paths.joined(separator: "、"))。本地改动尚未恢复\(stashDescription(recovery?.stashCommitSHA))。"
    case .stashRestoreConflict(let recovery, let paths):
      "变基已完成，恢复本地改动时遇到冲突：\(paths.joined(separator: "、"))。恢复 stash \(recovery.stashCommitSHA.prefix(12)) 已保留。"
    case .partial(let recovery, let message):
      "变基同步未完整结束：\(message)\(stashDescription(recovery?.stashCommitSHA))"
    }
  }
