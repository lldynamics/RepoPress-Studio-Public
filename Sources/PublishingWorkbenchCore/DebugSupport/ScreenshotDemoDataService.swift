  ...
  public struct ScreenshotDemoDataService {
    ...
    public static let knowledgeRootEnvironmentKey =
    ...
    public func makeSnapshot(now: Date = Date(timeIntervalSince1970: 1_900_000_000))
      -> WorkbenchSnapshot
    {
      ...
      let seoSnapshot = SEOSocialPreviewSnapshot(
        ...
        metaTags: [
          ...
          SEOSocialPreviewMetaTag(
            scope: .openGraph, property: "og:image", content: cover.relativePublishPath),
          SEOSocialPreviewMetaTag(
            scope: .twitter, property: "twitter:card", content: "summary_large_image"),
        ],
        findings: [
    ...
    public static func prepareKnowledgeLibraryServiceIfEnabled() -> KnowledgeLibraryService {
      ...
      let rootURL =
        configuredPath.flatMap { path in
          path.isEmpty ? nil : URL(fileURLWithPath: path, isDirectory: true)
        ...
        ?? (isUITest
          ? FileManager.default.temporaryDirectory
            .appendingPathComponent(
              "PersonalSitePublisherMac-AccessibilityUITestRepository", isDirectory: true)
    ...
    private static func seedSyncAPIPublishPreview(in store: WorkbenchStore) {
      ...
      store.setRepositoryTokenAvailability(
        KeychainTokenAvailability(
          hasToken: true, updatedAt: Date(timeIntervalSince1970: 1_900_000_000))
      )
      ...
      var preview =
        cachedSnapshot?.remotePublishPreview
    ...
    public static func seedKnowledgeLibraryIfEnabled(in knowledge: KnowledgeStore) async {
      guard isEnabledFromEnvironment,
        ...
        let sourceURL = URL(string: "https://example.com/accessibility/knowledge-library")
      else {
        return
      }
