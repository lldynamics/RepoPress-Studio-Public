...
public struct LocalGitPublishService: Sendable {
  ...
  public init(
    ...
    self.gitCommandRunner =
      gitCommandRunner ?? GitCommandRunner(executableURL: URL(fileURLWithPath: gitExecutablePath))
  }
  ...
  private func publish(
    ...
    let currentBranch = try trimmedOutput(
      runGit(["rev-parse", "--abbrev-ref", "HEAD"], rootURL: rootURL))
    ...
    do {
      ...
      return LocalGitPublishResult(
        ...
        output: outputChunks.map { $0.trimmedForPublishing }.filter { !$0.isEmpty }.joined(
          separator: "\n")
      )
    } catch {
  ...
  private func publishAsync(
    ...
    let currentBranch = try trimmedOutput(
      await runGitAsync(["rev-parse", "--abbrev-ref", "HEAD"], rootURL: rootURL))
    ...
    do {
      if mode == .reviewBranch {
        ...
        let switchResult = try await runGitAsync(
          ["switch", "-c", package.reviewBranchName], rootURL: rootURL)
        branchCreated = true
        commandLog.append(
          GitCommandRunner.redactedCommandDescription([
            "switch",
  ...
  private func ensurePublishPreconditions(packagePaths: [String], rootURL: URL) throws {
    ...
    guard status.isEmpty else {
      throw LocalGitPublishError.packagePathsNotClean(
  ...
  private func ensurePublishPreconditionsAsync(packagePaths: [String], rootURL: URL) async throws {
    ...
    if stagedResult.terminationStatus == 1 {
      let paths = try trimmedOutput(
        await runGitAsync(["diff", "--cached", "--name-only"], rootURL: rootURL)
      )
      .split(separator: "\n")
      .map(String.init)
      throw LocalGitPublishError.repositoryHasStagedChanges(paths)
  ...
  private func rollbackPublishAttempt(
    ...
    guard externallyModifiedPaths.isEmpty else {
      return [
        CoreL10n.format(
          "检测到发布后的外部修改，已停止自动回滚并保留当前文件：%@", externallyModifiedPaths.joined(separator: ", "))
      ]
    }
    ...
    if !appliedStatesByRepositoryPath.isEmpty {
      ...
      let originalPathsResult = attempt(
        ["ls-tree", "-r", "--name-only", initialHEAD, "--"] + packagePaths)
  ...
  private func rollbackPublishAttemptAsync(
    ...
    if !appliedStatesByRepositoryPath.isEmpty {
      ...
      let originalPathsResult = await attempt(
        ["ls-tree", "-r", "--name-only", initialHEAD, "--"] + packagePaths)
      let originalPaths = originalPathsResult.standardOutput.split(separator: "\n").map(String.init)
      if !originalPaths.isEmpty {
  ...
  private func externallyModifiedPackagePaths(
    ...
    appliedStatesByRepositoryPath.keys.sorted().filter { path in
      ...
      guard let currentState = try? localPublishFileState(at: currentURL, fileManager: fileManager)
      else {
        return true
      }
  ...
  private func ensureGitWorkTreeAsync(rootURL: URL) async throws {
    let result = await gitCommandRunner.runAsync(
      ["rev-parse", "--is-inside-work-tree"], rootURL: rootURL)
    guard result.terminationStatus == 0,
      trimmedOutput(result) == "true"
    else {
      throw LocalGitPublishError.notGitRepository(rootURL.path)
  ...
  public var errorDescription: String? {
    ...
    case .repositoryHasStagedChanges(let paths):
      return CoreL10n.format(
        "仓库已有暂存内容，请先提交或取消暂存后再发布：%@", paths.joined(separator: CoreL10n.text("、")))
    case .packagePathsNotClean(let entries):
      return CoreL10n.format(
        "发布目标路径存在未提交内容，为避免覆盖已停止发布：%@", entries.joined(separator: CoreL10n.text("；")))
    case .noStagedChanges:
