...
public struct AIPublishingAssistantService: Sendable {
  ...
  public func suggestImageText(
    ...
    visionInputs: [AIPublishingImageTextVisionInput] = [],
    profile: SiteProfile,
    config: AIProviderConfig,
    apiKey: String?
    ...
    if !visionInputs.isEmpty, !config.supportsImageInput {
      throw AIPublishingAssistantError.unsupportedImageAttachments(
        config.normalizedDisplayName
      )
    }
    let taskConfig = AIChatModelCatalog.config(for: .imageAltCaption, baseConfig: config)
    let targetIDs = Set(targetBatch.map(\.id))
    let preparedVisionInputs = Array(
      visionInputs
        .filter { targetIDs.contains($0.targetID) }
        .prefix(AIPublishingChatImageAttachmentPresentation.maxSelectedImageCount)
    )
    let prompt = imageTextPrompt(for: targetBatch, profile: profile)
    let userContent: AIChatMessageContent
    if preparedVisionInputs.isEmpty {
      userContent = .text(prompt)
    } else {
      var parts: [AIChatMessageContentPart] = [
        .text(
          prompt
            + "\n\n以下图片已由用户明确选择并随请求发送。每张图片前的 target_id "
            + "用于把视觉内容与 JSON 结果对应；只描述实际可见内容。"
        )
      ]
      for input in preparedVisionInputs {
        parts.append(
          .text(
            "target_id: \(input.targetID); filename: \(input.attachment.filename)"
          )
        )
        parts.append(.imageURL(input.attachment.dataURL))
      }
    ...
    let completion = AIChatCompletionRequest(
      ...
      messages: [
        AIChatMessage(
          role: "system",
          content: imageTextSystemPrompt(hasVisionInput: !preparedVisionInputs.isEmpty)
        ),
    ...
    let suggestions = AIPublishingImageTextSuggestionParser.parse(
      result.content, targets: targetBatch)
  ...
  public func prompt(for request: AIPublishingActionRequest) -> String {
    ...
    case .draftOpening:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成一个可直接插入正文开头的 Markdown 开头小节。开头要交代读者问题、文章会解决什么、适用边界和阅读收益；保持克制，不编造正文没有提供的事实。"
      )
    case .sharpenOpeningSelection:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请只优化下面选中的 Markdown 开头段。更快进入主题，明确文章要解决的问题、记录场景或读者收益；保留原意、事实、代码、链接和 Markdown 结构，不写营销式导语，不新增 front matter，只返回优化后的 Markdown 片段。"
      )
    case .draftOpeningHooks:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成 3 到 5 个可选开头钩子。每个钩子要具体、克制、贴合正文事实边界，并说明适合哪类读者。只输出 Markdown，不编造正文没有提供的事实。"
      )
    case .draftFullArticle:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前标题、摘要、标签和正文种子生成一版完整 Markdown 初稿。保留已有观点和事实边界，优先补结构、过渡、示例和结尾；信息不足的位置写“待补充”，不要编造事实、数据或真实链接。"
      )
    ...
    case .expandOutlineToDraft:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请把当前文章中的大纲、要点或草稿骨架扩写成连续 Markdown 正文。保留原有标题顺序和核心观点，补充必要过渡、解释、示例和限制条件；不要编造外部事实、真实数据、链接、账号或私密路径，不新增 front matter。"
      )
    ...
    case .draftArticleTLDR:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章上下文生成一个 Markdown 小节，标题为“## TL;DR”。用 3 到 5 条要点概括关键结论、限制条件和读者收益，不编造正文没有提供的事实。"
      )
    ...
    case .draftReaderQuestions:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成一个 Markdown 小节，标题为“## 读者可能会问”。给出 4 到 7 个读者可能追问的问题，每个问题后补一条简短说明，指出文章哪里还可以补充；问题必须围绕当前文章，不要泛泛而谈，不要编造事实。"
      )
    case .draftTransitionSection:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成一个可插入在两个主题之间的 Markdown 过渡段。说明上一部分和下一部分的关系、读者为什么需要继续看、哪些前提仍需保留；不要重复正文大段内容，不编造事实。"
      )
    case .draftExampleSection:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成一个示例小节，用来帮助读者理解正文已有概念、流程或判断。示例只能使用正文已经提供的事实和场景；缺少具体数据时写“待补充”，不要编造公司、人物、价格或真实案例。"
      )
    ...
    case .draftTutorialVersion:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请把当前文章改写成教程版 Markdown 草稿。保留已有事实和结论，按“适用场景 / 准备条件 / 操作步骤 / 验证方式 / 常见问题”组织；缺少命令、环境或验证结果时写“待确认”，不要编造工具输出，不新增 front matter。"
      )
    case .draftChecklistSection:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成一个 Markdown 检查清单小节。使用 - [ ] 任务列表格式，每一项都要具体、可检查，并围绕当前文章场景；不要加入与主题无关的通用事项，不要编造正文没有提供的任务，不新增 front matter。"
      )
    case .draftTroubleshootingSection:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成一个 Markdown 故障排查小节。按“现象 / 可能原因 / 如何确认 / 处理建议”组织；只使用正文已有信息，缺少日志、命令或环境时标为“待确认”，不要编造命令输出。"
      )
    case .draftCodeExample:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成一个代码示例小节。代码必须与正文技术语境一致，优先使用伪代码或最小可读示例；如果正文没有足够 API、语言或环境信息，请明确写出待确认项，不要编造可运行性。"
      )
    case .draftMermaidDiagram:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成一个 Mermaid 图示小节。只表达正文已经提供的流程、依赖或决策关系；节点名称要简短，缺失关系不要臆测。输出 Markdown fenced mermaid 代码块和一句使用说明。"
      )
    case .draftGlossary:
      return articleDraftPrompt(
        context: context,
        request: request,
        instruction:
          "请基于当前文章生成一个 Markdown 术语表小节。提取 5 到 12 个关键术语，每个术语用一行解释它在本文里的含义；只解释正文能支持的概念，不要编造正文没有支持的定义，不确定的术语标注“待确认”，不新增 front matter。"
      )
    ...
    case .draftInterviewQA:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成访谈式问答 Markdown 小节。生成 5 到 8 组问答，问题要像读者、作者复盘或采访中真实会问的问题；回答要短、具体、忠于正文，不要编造事实、承诺或外部来源。"
      )
    case .reorganizeStructure:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成结构重排建议。先指出当前结构的主要问题，再给出推荐的新段落顺序和每段写作要点；不要直接重写全文，不新增 front matter，不要编造事实，只返回可插入临时笔记区的 Markdown 建议。"
      )
    case .draftCounterpointSection:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章补充反方观点、限制和反例小节。列出 3 到 5 个可能的反方观点、边界条件、失败场景或不适用场景；每一点都要服务当前文章主题，避免为了辩论而编造事实。"
      )
    case .draftCaseStudySection:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成一个案例分析小节。案例要贴合当前主题，包含背景、做法、结果或复盘；不要编造真实客户、真实数据、真实账号或私密路径。上下文不足时使用“假设场景”并明确标注需要作者确认。"
      )
    ...
    case .polishSelection:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请只润色下面选中的 Markdown 文本，让表达更自然、清楚、克制。保留事实、代码、链接、Front Matter 片段和 Markdown 结构，不新增正文没有的事实，不添加解释。"
      )
    case .expandSelection:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请只扩写下面选中的 Markdown 文本，补足必要上下文、步骤、边界和读者理解所需说明。保留事实、代码、链接和 Markdown 结构，不编造正文没有提供的事实，不添加解释。"
      )
    case .continueAfterSelection:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请基于下面选中的 Markdown 文本续写后续内容，延续原文语气和上下文。只返回要插入到选区后面的 Markdown，不重复选中文本，不编造正文没有提供的事实。"
      )
    case .condenseSelection:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请只压缩下面选中的 Markdown 文本，删除重复和绕远表达，保留关键事实、限制条件、代码、链接和 Markdown 结构。输出可直接替换选区的 Markdown，不添加解释。"
      )
    case .removeRedundancySelection:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请只删减下面选中的 Markdown 文本里的冗余、重复、口水话和绕远表达。保留关键事实、判断依据、代码、链接和 Markdown 结构，输出可直接替换选区的 Markdown，不添加解释。"
      )
    case .checklistSelection:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请把下面选中的 Markdown 文本整理成可执行检查清单。使用 Markdown 任务列表，保留原有事实、步骤、代码和链接，不编造缺失条件；如果原文不适合转清单，请给出最接近的结构化清单。"
      )
    case .comparisonTableSelection:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请把下面选中的 Markdown 文本整理成 Markdown 对比表。只使用原文已经给出的维度、对象、条件和结论；缺失项写“待确认”，不编造事实，不添加表格外解释。"
      )
    case .explainSelection:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请解释下面选中的 Markdown 文本，补齐读者理解所需的前提、术语和边界。只返回要插入到选区后面的 Markdown 说明，不重复选中文本，不要编造正文没有提供的事实。"
      )
    case .simplifySelection:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请把下面选中的 Markdown 文本改写得更容易理解，降低术语密度，补足必要上下文。保留事实、代码、链接和 Markdown 结构，不改变结论，不编造事实，不添加解释。"
      )
    case .summarizeSelection:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请把下面选中的 Markdown 文本压缩成清晰摘要，保留关键事实、结论、限制条件、代码或链接引用。只返回可直接替换选区的 Markdown 摘要，不添加解释。"
      )
    ...
    case .draftBilingualRewrite:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请把下面选中的 Markdown 片段整理成中英文两个版本。保留代码、链接、图片引用、表格、专有名词和 Markdown 结构；中文版本自然克制，英文版本适合技术博客或个人网站阅读。如有不适合直译的表达，在末尾用“译法提醒”列出 1 到 3 条；不要新增 front matter，不扩展成完整文章。"
      )
    ...
    case .rewriteSelectionReaderFriendly:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请把下面选中的 Markdown 文本改写为读者友好版本，减少跳跃表达，增强衔接和可读性。保留事实、代码、链接和 Markdown 结构，不新增正文没有的事实，不添加解释。"
      )
    case .rewriteSelectionFormal:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请把下面选中的 Markdown 文本改写为更正式、克制、适合公开发布的语气。保留事实、代码、链接和 Markdown 结构，不新增正文没有的事实，不添加解释。"
      )
    case .rewriteSelectionCasual:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请把下面选中的 Markdown 文本改写为更轻松自然但仍适合个人网站发布的语气。保留事实、代码、链接和 Markdown 结构，不新增正文没有的事实，不添加解释。"
      )
    case .rewriteSelectionTechnical:
      return selectedTextPrompt(
        ...
        request: request,
        instruction:
          "请把下面选中的 Markdown 文本改写为更适合技术读者的表达，强化术语准确性、步骤边界和工程语境。保留事实、代码、链接和 Markdown 结构，不新增正文没有的事实，不添加解释。"
      )
    ...
    case .suggestTitles:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成 5 个标题候选，每行一个。标题要具体、清楚，适合静态博客文章；不要使用引号、编号解释、front matter 或正文，不要夸大文章价值。"
      )
    case .suggestSlug:
      return articleDraftPrompt(
        context: context,
        request: request,
        instruction:
          "请基于当前文章生成 5 个 slug 候选，每行一个。使用小写 kebab-case，只包含小写英文字母、数字和连字符；不要包含日期、文件扩展名、路径分隔符、引号、编号、front matter 或解释。slug 要短、稳定、可读。"
      )
    case .suggestSummary:
      return articleDraftPrompt(
        context: context,
        request: request,
        instruction:
          "请基于当前文章生成一条 summary/description。摘要要具体、克制，适合静态博客列表页、RSS 和社交分享；长度控制在 60 到 160 个中文字符，或 25 到 45 个英文单词。不要使用引号、编号、Markdown、front matter 或解释，只返回一条摘要文本。"
      )
    case .suggestTags:
      return articleDraftPrompt(
        context: context,
        request: request,
        instruction:
          "请基于当前文章生成 3 到 8 个 tags 候选，每行一个。tags 要短、稳定、可复用，避免泛泛的“随笔”“记录”等弱标签；优先复用当前文章已有技术名词、项目名和主题名。不要使用编号、引号、Markdown、front matter 或解释。"
      )
    ...
    case .reviewContentGaps:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请检查当前文章的内容缺口。按“缺口 / 为什么影响发布 / 需要补充的证据或段落 / 建议插入位置”输出 Markdown 表格。不要把未提供的事实当成已经存在。"
      )
    case .flagUnsupportedClaims:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成事实边界提醒，输出 Markdown 小节标题“## 事实边界提醒”。标出可能缺少来源、过度概括、数字或结论无依据、因果关系跳跃、效果承诺过强的句子；不要联网查证，不要替作者补外部来源。每条给出“为什么需要确认”和“建议怎么改得更稳”；没有明显问题时写“未发现明显事实边界问题”。"
      )
    case .draftSourceChecklist:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请为当前文章生成来源补充清单。按“需要来源的位置 / 应验证的问题 / 合适来源类型 / 临时保守表述”输出 Markdown 表格。不要编造真实链接、论文、官方文档或已经完成的核验。"
      )
    case .suggestInternalLinks:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请给当前文章生成内链建议。只能基于上下文里已经出现的站点结构、发布路径、标签、分类和文章主题推导；不要编造不存在的真实路径。若没有足够站内文章信息，请输出需要补充的候选文章清单标准。"
      )
    case .auditLinkQuality:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请检查当前文章中的链接质量。按“链接或引用位置 / 风险 / 如何人工确认 / 建议改法”输出 Markdown 表格。不要声称已经访问外部网页，不要编造 HTTP 状态、发布时间或页面内容。"
      )
    case .auditImagePrivacy:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章和图片上下文生成图片隐私检查，输出 Markdown 小节标题“## 图片隐私检查”。检查 Markdown 图片引用、封面、文件名、路径和图片工作台信息里可能暴露的个人信息、客户信息、内部界面、地理位置、账号、token、私密路径或未公开业务内容。不要假装看到了图片真实画面；只能基于文件名、引用位置、正文上下文和图片检查信息判断，不确定的地方标“需要人工确认”。"
      )
    case .reviewSSGCompatibility:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请检查当前文章对静态博客框架的兼容性，覆盖 Hexo、Hugo、Zola、Astro、Jekyll 常见 front matter、摘要、图片路径、短代码和 Markdown 语法风险。只基于当前文章和站点配置判断，缺少信息写“待确认”。"
      )
    ...
    case .reviewReaderClarity:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请从目标读者角度检查当前文章清晰度。输出读者可能卡住的位置、缺少的前提、跳跃的段落、术语解释建议和可直接插入的过渡句。不要改变文章结论，不要补正文没有依据的事实。"
      )
    case .reviewTechnicalAccuracy:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请检查当前文章的技术准确性风险。按“表述 / 风险类型 / 需要验证的证据 / 建议保守改写”输出 Markdown 表格。不要假装运行过代码、命令或测试；无法确认的地方明确写“待验证”。"
      )
    case .draftImageAltCaptions:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章和图片上下文生成图片 alt/caption 建议。不能假装看到了图片内容；只能根据文件名、引用位置、正文上下文和图片检查信息推断。输出每张图片的 alt、caption、保守理由和需要人工确认的点。"
      )
    case .draftSocialShare:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成社交分享文案。分别给出 X/微博短文案、长文案、LinkedIn/公众号式摘要和 3 个可选开头。不要夸大结论，不要编造读者反馈或发布数据。"
      )
    case .draftPublishAssetPack:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请生成发布素材包：SEO 标题备选、社交摘要、摘录卡片文案、Newsletter 摘要、图片 alt/caption 注意项和发布检查清单。所有内容必须基于当前文章，不要编造验证结果。"
      )
    case .draftPullQuotes:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请从当前文章中提炼可引用摘录。输出 6 到 10 条短摘录，每条包含适用渠道、原文依据位置和是否需要人工确认。只能改写正文已经表达的观点，不要制造更强结论或虚构金句。"
      )
    case .draftPublishNote:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成发布说明和提交文案，输出 Markdown 小节标题“## 发布说明”。包含一句发布摘要、3 到 5 条变更要点、一个简短 commit message 候选。语气要具体、克制，适合个人网站文章发布或 Git 提交；不要凭空补充正文没有体现的改动，不要新增 front matter。"
      )
    case .draftNewsletterSummary:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成 Newsletter 摘要。包含一句话导读、3 条要点、适合邮件开头的短段落和一个克制 CTA。不要夸大价值，不要编造用户反馈、阅读量或外部验证。"
      )
    ...
    case .draftCrossPlatformAnnouncement:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成跨平台发布摘要，输出 Markdown 小节标题“## 跨平台发布摘要”。包含网站列表页摘要、RSS 摘要、社交短文、较长社交说明和 commit message 候选。每个版本都要具体、克制，忠于文章内容；不要营销化、不要夸大效果、不要新增 front matter。"
      )
    case .draftShortVideoScript:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成短视频口播稿，输出 Markdown 小节标题“## 短视频口播稿”。包含 15 秒、30 秒、60 秒三个版本，每个版本都要有开场句、核心要点和结尾引导。只使用当前文章能支持的信息；不要编造演示画面、用户反馈、数据、外部验证或夸张承诺。"
      )
    ...
    case .draftCommentReply:
      return articleDraftPrompt(
        ...
        request: request,
        instruction:
          "请基于当前文章生成评论回复草稿，输出 Markdown 小节标题“## 评论回复草稿”。给出 3 种回复：感谢补充、澄清误解、引导继续阅读或后续更新。每种回复都要短、自然、可复制，避免争辩式语气；不要编造评论者身份、外部事实、链接或文章中不存在的承诺。"
      )
  ...
  private var systemPrompt: String {
    "你是RepoPress里的发布上下文助手。你不做泛聊天，只围绕当前文章、站点结构、front matter、SEO、公开风险、图片和发布说明给建议。只输出给用户的最终答复，不得展示思考、推理、权衡、草稿或内部决策过程；信息不足时直接、简短地说明。"
  }
  ...
  private func imageTextSystemPrompt(hasVisionInput: Bool) -> String {
    let evidenceInstruction =
      hasVisionInput
      ? "用户已明确选择并发送实际图片。应以可见画面为主要依据，并结合文章上下文判断图片作用；看不清或无法确认的细节必须使用保守表述。"
