...
struct AIComponentProcessResult: Sendable {
  let output: String
  let terminationStatus: Int32?
  let interruption: String?
  var succeeded: Bool { terminationStatus == 0 && interruption == nil }
}

final class AIComponentProcessRunner: @unchecked Sendable {
  private static let maximumOutputByteCount = 1_048_576
  private static let maximumReturnedCharacterCount = 8_192
  ...
  private var continuation: CheckedContinuation<AIComponentProcessResult, Never>?
  private var completedResult: AIComponentProcessResult?
  ...
  func run(timeout: Duration) async -> AIComponentProcessResult {
    await withTaskCancellationHandler {
      await runProcess(timeout: timeout)
    } onCancel: {
      self.stop(.cancelled)
    }
  }

  private func runProcess(timeout: Duration) async -> AIComponentProcessResult {
  ...
  private func consumeOutput(_ data: Data) {
    ...
    if !data.isEmpty { onOutput(String(decoding: data.prefix(8_192), as: UTF8.self)) }
    if shouldStop {
      stop(.outputLimit)
    } else {
      completeIfReady()
  ...
  private func launchProcess() throws -> pid_t {
    let arguments = try CodexRuntimeCStringArray([executableURL.path] + self.arguments)
    ...
    guard posix_spawn_file_actions_init(&fileActions) == 0 else {
      throw AIComponentProcessRunnerLaunchError.failed
    }
  ...
  private func requestProcessGroupCleanup(_ processIdentifier: pid_t) {
    ...
    DispatchQueue.global(qos: .utility).asyncAfter(
      deadline: .now() + Self.forceKillDelay.componentProcessTimeInterval
    ) { [self] in
  ...
  private func completeIfReady() {
    let continuation: CheckedContinuation<AIComponentProcessResult, Never>?
    let result: AIComponentProcessResult
  ...
  private func makeResultLocked() -> AIComponentProcessResult {
    AIComponentProcessResult(
      output: String(
        String(decoding: output, as: UTF8.self).suffix(Self.maximumReturnedCharacterCount)),
      terminationStatus: terminationStatus,
      interruption: stopReason?.rawValue
    )
  }

  private func result() async -> AIComponentProcessResult {
...
private enum AIComponentProcessRunnerLaunchError: Error {
  case failed
}

extension Duration {
  fileprivate var componentProcessTimeInterval: TimeInterval {
