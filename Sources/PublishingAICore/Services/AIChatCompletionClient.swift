...
public struct AIChatCompletionClient: Sendable {
  ...
  package func authorizingNonStreamingRequests(
    _ authorization: @escaping @Sendable () async throws -> Void
  ) -> AIChatCompletionClient {
    var copy = self
    let previous = nonStreamingRequestAuthorization
    copy.nonStreamingRequestAuthorization = {
      try await previous?()
  ...
  package func withCodexAppServerRequestAuthorizer(
    _ authorizer: (any CodexAppServerRequestAuthorizing)?
  ) -> AIChatCompletionClient {
    var copy = AIChatCompletionClient(
      transport: transport,
      encoder: encoder,
      decoder: decoder,
