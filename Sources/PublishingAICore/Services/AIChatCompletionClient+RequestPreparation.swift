...
extension AIChatCompletionClient {
  ...
  private func performPreparedDataTransport(
    ...
    do {
      ...
      // A revoked request must never enter network recovery/replay.
      throw CancellationError()
    }
    try Task.checkCancellation()
  ...
  func makePartialTextContinuationPrepared(
    ...
    let continuationInstruction = AIChatMessage(
      role: "user",
      content:
        "Continue directly from the assistant checkpoint above. Do not repeat any text already shown; write only the missing continuation."
    )
