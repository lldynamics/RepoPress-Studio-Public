...
public struct LocalSitePreviewDiagnostics: Codable, Hashable, Sendable {
  ...
  public init(
    ...
    self.issues = issues
    self.isReadyToStart =
      isReadyToStart ?? !dependencies.contains(where: { $0.status.isBlocking })
...
/// One bounded readiness policy shared by the browser hand-off and the
/// background loopback probe. Quartz first copies a repository and then builds
/// it, so its page may legitimately appear later than an ordinary dev server.
public enum LocalSitePreviewStartupBudget {
  public static let normalMaximumReadinessAttempts = 12
  public static let normalBackgroundReachabilityAttempts = 5
  public static let quartzCopyTimeoutSeconds = 120
  public static let quartzBuildTimeoutSeconds = 120
  public static let quartzStartupGraceSeconds = 5

  public static func maximumReadinessAttempts(for siteKind: SiteKind) -> Int {
    guard siteKind == .quartz else { return normalMaximumReadinessAttempts }
    // Initial retry delays are shorter than one second. Leave enough attempts
    // for the wall-clock deadline, rather than ending during startup grace.
    return quartzCopyTimeoutSeconds + quartzBuildTimeoutSeconds + quartzStartupGraceSeconds
      + normalMaximumReadinessAttempts
  }

  public static func maximumReadinessWait(for siteKind: SiteKind) -> Duration {
    guard siteKind == .quartz else { return .seconds(normalMaximumReadinessAttempts) }
    return .seconds(
      quartzCopyTimeoutSeconds + quartzBuildTimeoutSeconds + quartzStartupGraceSeconds
    )
  }
}
