...
public enum LocalSitePreviewExecutionFingerprint {
  static let maximumManifestByteCount = 1 * 1_024 * 1_024

  public struct ManifestSnapshot: Sendable {
  ...
  public static func manifestRelativePaths(for siteKind: SiteKind) -> [String] {
    switch siteKind {
    case .astro, .vitePress, .nextJS, .hexo, .docusaurus:
      return ["package.json"]
    case .quartz:
      return [
        "package.json", "quartz.config.ts", "quartz.layout.ts", "quartz/bootstrap-cli.mjs",
      ]
    case .jekyll:
      return ["Gemfile", "Gemfile.lock"]
    case .zola, .hugo, .foam, .mkDocs:
      return []
    }
  }

  public static func captureManifest(
    rootPath: String,
    siteKind: SiteKind
...
public final class LocalSitePreviewTrustStore: @unchecked Sendable {
  ...
  public func isAuthorized(_ identity: LocalSitePreviewExecutionIdentity) -> Bool {
    lock.lock()
    defer { lock.unlock() }
  ...
  public func authorize(_ identity: LocalSitePreviewExecutionIdentity) throws {
    lock.lock()
    defer { lock.unlock() }
    ...
    do {
      ...
      // Preserve unreadable, corrupt, and newer-schema records for recovery.
      // Confirmation cannot silently replace another site's trust data.
  ...
  public func invalidate(_ identity: LocalSitePreviewExecutionIdentity) {
    lock.lock()
    defer { lock.unlock() }
    do {
  ...
  /// Uses a single authorization exactly once. A failed write must prevent
  /// execution, since otherwise the old grant could be replayed later.
  public func consume(_ identity: LocalSitePreviewExecutionIdentity) throws {
    lock.lock()
    defer { lock.unlock() }
    do {
      var document = try loadDocument()
      guard document.records.contains(where: { $0.matches(identity) }) else {
        throw LocalSitePreviewError.authorizationRequired
      }
      document.records.removeAll { $0.matches(identity) }
      try write(document)
    } catch let error as LocalSitePreviewError {
      throw error
    } catch {
      throw LocalSitePreviewError.authorizationStoreUnavailable(error.localizedDescription)
    }
