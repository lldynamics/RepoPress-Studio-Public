...
struct RSSReaderView: View {
  ...
  private func recordReadingProgress(_ value: Double, for articleID: String) {
    ...
    guard RSSReadingCompletionPolicy.shouldAutomaticallyMarkRead(progress: normalized),
          store.articleHeader(id: articleID)?.isRead == false else {
      return
    }
    store.markRead(articleID)
    if selectedArticlePayload?.id == articleID {
...
private struct RSSArticleList: View {
  ...
  private func articleRow(_ article: RSSArticleHeader, feed: RSSFeed?) -> some View {
    let coverAsset = coverAsset(for: article)
    return RSSArticleRow(
      article: article,
      feed: feed,
      summary: article.readableSummary,
      coverAsset: coverAsset,
      coverLocalURL: coverAsset?.localURL(in: store.mediaCacheDirectoryURL),
      allowsPrivateNetworkAccess: store.privateNetworkAccessEnabled,
  ...
  private func coverAsset(for article: RSSArticleHeader) -> RSSMediaAsset? {
    let assets = store.mediaAssets.filter { asset in
      guard asset.articleID == article.id else { return false }
      if let contentType = asset.contentType?.lowercased(), !contentType.isEmpty {
        return contentType.hasPrefix("image/")
      }
      return ["avif", "bmp", "gif", "jpeg", "jpg", "png", "svg", "webp"]
        .contains(asset.remoteURL.pathExtension.lowercased())
    }
    if let coverURL = article.coverURL {
      return assets.first { $0.remoteURL == coverURL }
    }
    return assets.first
  }
}
...
private struct RSSArticleRow: View {
  ...
  let feed: RSSFeed?
  let summary: String
  let coverAsset: RSSMediaAsset?
  let coverLocalURL: URL?
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 0) {
      ...
      RSSArticleCoverThumbnail(
        remoteURL: article.coverURL ?? coverAsset?.remoteURL,
        localURL: coverLocalURL,
...
private struct RSSArticleCoverThumbnail: View {
  let remoteURL: URL?
  let localURL: URL?
  let allowsPrivateNetworkAccess: Bool
  ...
  private var sourceKey: String {
    [localURL?.absoluteString, remoteURL?.absoluteString, String(allowsPrivateNetworkAccess)]
      .compactMap { $0 }
      .joined(separator: "|")
  }
  ...
  private func loadImage() async {
    ...
    if let localURL,
       let localImage = await WorkbenchImageTwoTierCache.shared.thumbnail(
         for: localURL,
         maxPixelSize: 128
       ) {
      image = localImage
    ...
    do {
      let data = try await RSSArticleCoverImageLoader(
        allowsPrivateNetworkAccess: allowsPrivateNetworkAccess
      ).load(from: remoteURL)
      guard !Task.isCancelled else { return }
      image = NSImage(data: data)
    } catch {
      // A failed cover is non-critical; keep the compact placeholder and the
      // article text fully usable.
    }
  }
}

private struct RSSReadingProgressBar: View {
  let progress: Double
    ...
    GeometryReader { geometry in
      ZStack(alignment: .leading) {
        ...
        Rectangle()
          ...
          .frame(width: geometry.size.width * CGFloat(normalizedProgress))
      }
    }
    .frame(height: 2)
    .accessibilityElement(children: .ignore)
    .accessibilityLabel("阅读进度")
    .accessibilityValue("已读 \(percentage)%")
    ...
    return VStack(spacing: 0) {
      ...
      if !hasRenderableBody {
        ...
        ZStack(alignment: .top) {
          ...
          RSSReadingProgressBar(progress: normalizedReadingProgress)
        }
        .frame(maxWidth: 900, maxHeight: .infinity, alignment: .topLeading)
