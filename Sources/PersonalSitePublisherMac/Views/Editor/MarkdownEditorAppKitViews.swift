...
final class MarkdownEditorScrollView: NSScrollView {
  ...
  override func viewDidEndLiveResize() {
    ...
    pendingLiveResizeAnchor =
      session.text == currentText
  ...
  override func layout() {
    ...
    let restoredOrigin =
      widthChanged && !defersWidthChangeForLiveResize
  ...
  func completeDeferredFrameReflow() {
    ...
    pendingLiveResizeAnchor =
      abs(contentView.bounds.minY - deferredFrameReflow.viewportOriginY) > 0.5
      ? MarkdownEditorViewportAnchor.capture(in: self)
    ...
    private func startPerformanceAutoScroll() {
      ...
      performanceScrollCycleCount =
        performanceScrollPattern == .forward
        ? 1
    ...
    @objc private func performPerformanceAutoScrollStep() {
      ...
      let fraction =
        cycleProgress >= Double(performanceScrollCycleCount)
        ? 1
    ...
    @objc private func performPerformanceAutoTypingStep() {
      ...
      guard
        let edit = performanceAutoTypingEdit(
...
final class DroppableMarkdownTextView: NSTextView {
  ...
  override func accessibilityChildren() -> [Any]? {
    ...
    let markdownChildren: [Any] =
      markdownTaskCheckboxAccessibilityElements.map { $0 as Any }
  ...
  private func updateMarkdownInlineAttachmentAccessibilityElements() {
    ...
    for drawing in markdownInlineAttachmentDrawings.values.sorted(by: {
      ...
      let element =
        markdownInlineAttachmentAccessibilityElementsByKey[drawing.key]
