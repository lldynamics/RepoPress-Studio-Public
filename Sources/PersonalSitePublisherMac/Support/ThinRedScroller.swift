...
private final class ThinRedScrollbarsConfiguratorView: NSView {
  ...
  private func acceptsTargetWindow(_ targetWindow: NSWindow) -> Bool {
    ...
    // Both scopes cover the owning window plus any sheet it presents, so
    // sheets never fall back to the system default scroller.
...
enum ThinRedScrollbarWebStyle {
  ...
  /// JavaScript that injects `css` into an already-rendered document. Used for
